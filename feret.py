...
def adjustBox(obj,edge_points,boundingBox,primky):
    ...
    for i in range(0,4):
        if(liesIn(obj,boundingBox[i],boundingBox[(i+1)%4],primky[i])):#přímka leží v objektu
            fixVec=getAdjustVector(edge_points, primky[i], boundingBox[i], boundingBox[(i+1)%4], primky[(i+2)%4], boundingBox[(i+3)%4], 1)
...
def getFeretDiameters(obj):
    ...
    for angle in range(0,90,step):#budu rotovat obdélník po 2 stupních celkem o pí/2
        print(angle)    
